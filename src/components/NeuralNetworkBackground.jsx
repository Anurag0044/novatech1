...
const NeuralNetworkBackground = () => {
  ...
  // ===== CONFIGURATION - CHANGE ACTIVE COLOR THEME HERE =====
  const config = {
    paused: false,
    activePaletteIndex: 0, // CHANGE THIS NUMBER (0-3) to switch color themes
    currentFormation: 0,
    numFormations: 4,
    densityFactor: 1
  };

  // ===== COLOR PALETTES - CHANGE COLORS HERE =====
  // You can modify these color arrays to change the neural network colors
  // Each array represents a different color theme
  // Format: [color1, color2, color3, color4, color5] in hex format (0xRRGGBB)
  const colorPalettes = [
    // Theme 1: Purple/Blue gradient
    [new THREE.Color(0xDB2777), new THREE.Color(0xC026D3), new THREE.Color(0x4F46E5), new THREE.Color(0x7C3AED), new THREE.Color(0x8B5CF6)],
    // Theme 2: Orange/Red gradient (CURRENTLY ACTIVE - config.activePaletteIndex = 1)
  ...
  const generateNeuralNetwork = (formationIndex, densityFactor = 1.0) => {
    ...
    switch (formationIndex % 4) {
      ...
      default: generateQuantumCortex(); break;
    }
  ...
  const initializeThreeJS = () => {
    ...
    return { scene, camera, renderer, controls, composer, starField };
  };
  ...
  const createNetworkVisualization = (formationIndex, densityFactor = 1.0) => {
    ...
    const existingMeshes = scene.children.filter(child => 
      child.userData.isNeuralNetwork
    );
    ...
    neuralNetwork.nodes.forEach((node, index) => {
      ...
      // Add slight color variations for visual interest
      baseColor.offsetHSL(
    ...
    nodesMesh.userData.isNeuralNetwork = true;
    scene.add(nodesMesh);
    ...
    connectionsMesh.userData.isNeuralNetwork = true;
    scene.add(connectionsMesh);

    // ===== APPLY COLOR PALETTE TO SHADERS =====
    const palette = colorPalettes[config.activePaletteIndex];
    // Set pulse colors for connections
  ...
  const triggerPulse = (clientX, clientY) => {
    ...
    // Create an interaction plane in front of the camera
    ...
    if (raycasterRef.current.ray.intersectPlane(interactionPlane, interactionPoint)) {
      ...
      // Find neural network meshes and update pulse uniforms
      const neuralMeshes = sceneRef.current.children.filter(child => child.userData.isNeuralNetwork);
      neuralMeshes.forEach(mesh => {
        if (mesh.material.uniforms && mesh.material.uniforms.uPulsePositions) {
          lastPulseIndexRef.current = (lastPulseIndexRef.current + 1) % 3;
          
          mesh.material.uniforms.uPulsePositions.value[lastPulseIndexRef.current].copy(interactionPoint);
          mesh.material.uniforms.uPulseTimes.value[lastPulseIndexRef.current] = time;

          // ===== SET PULSE COLOR FROM CURRENT PALETTE =====
          const palette = colorPalettes[config.activePaletteIndex];
          const randomColor = palette[Math.floor(Math.random() * palette.length)];
          mesh.material.uniforms.uPulseColors.value[lastPulseIndexRef.current].copy(randomColor);
        }
      });
    }
  };
  ...
  const handleTouchStart = (event) => {
    event.preventDefault();
    if (event.touches.length > 0 && !config.paused) {
      triggerPulse(event.touches[0].clientX, event.touches[0].clientY);
    }
  };

  const handleTouchEnd = (event) => {
    event.preventDefault();
    // Re-enable auto-rotate after touch ends
    setTimeout(() => {
      if (controlsRef.current) {
        controlsRef.current.autoRotate = true;
      }
  ...
  const animate = () => {
    if (!rendererRef.current || !controlsRef.current || !composerRef.current) return;
    ...
    if (!config.paused) {
      ...
      neuralMeshes.forEach(mesh => {
        ...
        mesh.rotation.y = Math.sin(t * 0.05) * 0.08;
      });
    }

    const starField = sceneRef.current.children.find(child => child.type === 'Points');
    if (starField) {
  ...
  useEffect(() => {
    const init = () => {
      const threeJS = initializeThreeJS();
      if (threeJS) {
